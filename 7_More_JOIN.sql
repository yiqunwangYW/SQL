...
WHERE movieid IN (
  SELECT movieid FROM actor JOIN casting ON actor.id=actorid 
            JOIN movie ON movieid=movie.id WHERE name='Julie Andrews') ;
                
--13. Obtain a list, in alphabetical order, of actors who've had at least 30 
-- starring roles.
SELECT name 
FROM actor JOIN casting ON (id = actorid AND (SELECT COUNT(ord) FROM casting 
                                              WHERE actorid = actor.id AND ord=1)>=30)
                                              GROUP BY name;
--or
SELECT name 
FROM actor JOIN casting ON (id = actorid AND (SELECT sum(CASE WHEN ord=1 THEN 1 ELSE 0 END)>=30 
                                              FROM casting WHERE actorid=id)) 
                                              GROUP BY name;
